...
$old = [
    'quote_no' => quotation_next_number(),
    'customer_id' => '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($old['quote_no'] === '') {
        ...
        $checkStmt = $pdo->prepare('SELECT id FROM quotations WHERE branch_id = ? AND quote_no = ? LIMIT 1');
        $checkStmt->execute([$branchId, $old['quote_no']]);
        ...
        try {
            ...
            $quoteStmt = $pdo->prepare('
                INSERT INTO quotations(
                    branch_id, customer_id, user_id, quote_no, status, valid_until,
            ...
            log_activity($pdo, 'create_quotation', 'quotations', 'Created quotation ' . $old['quote_no']);
            $pdo->commit();

            $_SESSION['flash'] = ['type' => 'success', 'message' => 'Quotation created successfully.'];
            redirect_to('quotations/view.php?id=' . $quotationId);
        } catch (Throwable $e) {
            if ($pdo->inTransaction()) {
                $pdo->rollBack();
            }
            $errors[] = 'Quotation could not be saved. ' . $e->getMessage();
        }
    }
}
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-3">
    <div>
        ...
        <small class="text-muted">Build a quote from current branch products. Stock is checked only when it becomes a sale.</small>
    </div>
...
<?php if ($errors): ?>
    <div class="alert alert-danger">
        <strong>Quotation was not saved.</strong>
        <ul class="mb-0">
...
<?php if (!$products): ?>
    <div class="alert alert-warning">Add at least one product in this branch before creating quotations.</div>
<?php endif; ?>

<form method="post" action="<?= app_url('quotations/create.php') ?>" class="table-card">
    <div class="row g-3 mb-3">
        ...
        <div class="col-md-4">
            ...
            <?php if (!$canViewCustomers): ?>
                <small class="text-muted">This role cannot select customers.</small>
            <?php endif; ?>
        </div>
        <div class="col-md-2">
        ...
        <div class="col-12">
            ...
            <textarea name="notes" class="form-control" rows="2" maxlength="2000" placeholder="Terms, validity notes, or customer requests"><?= htmlspecialchars($old['notes']) ?></textarea>
        </div>
    </div>
    ...
    <div class="d-flex justify-content-end gap-2 mt-4">
        <a class="btn btn-outline-secondary" href="<?= app_url('quotations/index.php') ?>">Cancel</a>
        <button class="btn btn-primary" <?= !$products ? 'disabled' : '' ?>>
